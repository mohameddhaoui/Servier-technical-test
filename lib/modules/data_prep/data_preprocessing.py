...
def run_data_preprocessing_pipeline(
    df_filepath: str,
    datasrc_name: str,
    data_preprocessing_config: dict,
    ...
    if is_success(dependency):
        try:
            ...
            output_preprocessed_filepath = (
                f"{output_directory}{os.path.basename(df_filepath)}"
            )
...
def _transform_data(df_filepath: str, datasrc_name: str, data_preprocessing_config: dict) -> dict:
    ...
    if "column_transformation" in data_preprocessing_config.keys():
        df_data_transformation = _transform_columns(
            df_data_transformation,
